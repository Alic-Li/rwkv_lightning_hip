...
std::vector<uint16_t> copy_half_buffer_to_host(const rwkv7_fast_v4::DeviceBuffer<half>& buffer) {
  ...
  if (buffer.n > 0) {
    check_cuda(
        hipMemcpy(host.data(), buffer.p, buffer.n * sizeof(uint16_t), hipMemcpyDeviceToHost),
...
std::vector<float> copy_float_buffer_to_host(const rwkv7_fast_v4::DeviceBuffer<float>& buffer) {
  ...
  if (buffer.n > 0) {
    check_cuda(
        hipMemcpy(host.data(), buffer.p, buffer.n * sizeof(float), hipMemcpyDeviceToHost),
...
void copy_half_buffer_to_device(
  ...
  if (!host.empty()) {
    check_cuda(
        hipMemcpy(buffer.p, host.data(), host.size() * sizeof(uint16_t), hipMemcpyHostToDevice),
        label);
  }
}
...
void copy_float_buffer_to_device(
  ...
  if (!host.empty()) {
    check_cuda(
        hipMemcpy(buffer.p, host.data(), host.size() * sizeof(float), hipMemcpyHostToDevice),
        label);
  }
}
...
std::shared_ptr<GenerationState> StateCacheManager::clone_device_state(const GenerationState& state) const {
  ...
  if (state.shift.n > 0) {
    check_cuda(
        hipMemcpy(clone->shift.p, state.shift.p, state.shift.n * sizeof(half), hipMemcpyDeviceToDevice),
        "clone shift memcpy");
  }
  if (state.wkv32) {
