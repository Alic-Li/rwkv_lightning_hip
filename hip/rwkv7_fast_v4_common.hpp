...
inline void ignore_hip_error(hipError_t err) {
  (void)err;
}
...
struct DeviceBuffer {
  ...
  void resize(std::size_t count, const char* what) {
    ...
    if (p) {
      check_cuda(hipFree(p), what);
    }
    n = count;
    check_cuda(hipMalloc(&p, n * sizeof(T)), what);
  }

  void zero(const char* what) {
    if (p && n) {
      check_cuda(hipMemset(p, 0, n * sizeof(T)), what);
    }
  }
};
